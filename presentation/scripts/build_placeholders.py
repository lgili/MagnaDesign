...
from matplotlib.patches import (  # noqa: E402
    FancyBboxPatch, Rectangle, Wedge,
)
...
# -------------------------------------------------------------------
def render_spec_drawer(out: Path, title: str,
    ...
    card = FancyBboxPatch(
        ...
        linewidth=1.2, edgecolor=P_BORDER, facecolor=P_SURFACE,
    )
    ax.add_patch(card)

    # Header bar — coloured rule + section title.
    ax.add_patch(Rectangle((0.04, 0.86), 0.92, 0.005,
                           color=P_VIOLET, linewidth=0))
    ax.text(0.07, 0.88, title, fontsize=14, fontweight="bold",
            color=P_TEXT, va="bottom")
    # Topology pill.
    pill_x, pill_y, pill_w, pill_h = 0.74, 0.875, 0.20, 0.045
    ax.add_patch(FancyBboxPatch(
        (pill_x, pill_y), pill_w, pill_h,
        boxstyle="round,pad=0.0,rounding_size=0.025",
        linewidth=0, facecolor=P_VIOLET_DARK,
    ))
    ax.text(pill_x + pill_w / 2, pill_y + pill_h / 2,
            topology, fontsize=9, fontweight="bold",
    ...
    for i, (label, value) in enumerate(col1):
        y = y_start - i * line_h
        ax.text(0.08, y, label.upper(), fontsize=8,
                color=P_MUTED, fontweight="bold")
        ax.text(0.08, y - label_to_value, value, fontsize=11,
                color=P_TEXT, family="monospace")
    for i, (label, value) in enumerate(col2):
        y = y_start - i * line_h
        ax.text(0.55, y, label.upper(), fontsize=8,
                color=P_MUTED, fontweight="bold")
        ax.text(0.55, y - label_to_value, value, fontsize=11,
                color=P_TEXT, family="monospace")

    # Footer note — small caption.
    ax.text(0.5, 0.10,
            "Press ENTER on any field to recalculate the design.",
            fontsize=8, color=P_MUTED, style="italic", ha="center")
...
def render_fea_dispatch(out: Path) -> None:
    ...
    def box(x, y, w, h, text, color=P_VIOLET_DARK,
            fill=P_SURFACE, fontsize=10, bold=False):
        ax.add_patch(FancyBboxPatch(
            (x, y), w, h,
        ...
        ax.text(x + w / 2, y + h / 2, text, fontsize=fontsize,
                color=P_TEXT, fontweight=weight,
                ha="center", va="center")
    ...
    def arrow(x1, y1, x2, y2, label=None, color=P_VIOLET_DARK):
        ...
        if label:
            ...
            ax.text(mx, my, label, fontsize=8, color=P_MUTED,
                    bbox=dict(facecolor=P_BG, edgecolor="none",
                              pad=2),
    ...
    box(0.5, 2.8, 3, 0.9,
        "FEMMT path\n(N > 150 → would crash)",
        color=P_DANGER, fill="#FEF2F2")
    box(6.5, 2.8, 3, 0.9,
        "FEMM legacy\nbulk current region",
        color=P_SUCCESS, fill="#F0FDF4")

    arrow(4.5, 4.8, 2.0, 3.7, "shape = toroid?")
    arrow(5.5, 4.8, 8.0, 3.7, "auto-fallback")

    box(6.5, 0.6, 3, 1.1,
        "Result returned\nNo error to user", bold=True,
        color=P_VIOLET_DARK, fill="#F5F3FF")
    arrow(8.0, 2.8, 8.0, 1.7)

    ax.text(2.0, 2.0,
            "Would skip with\n‘FEA skipped: N exceeds…’",
            fontsize=9, color=P_DANGER, ha="center", va="top",
            style="italic")

    ax.text(5, 0.1,
            "FEMM legacy models the winding as a homogeneous current "
            "region — N has zero geometric cost.",
            fontsize=9, color=P_MUTED, ha="center", style="italic")
...
def render_pareto(out: Path) -> None:
    ...
    ax.scatter(losses, cost, s=14, alpha=0.18, color=P_MUTED,
               edgecolors="none",
               label=f"Dominated  (n = {n - len(front_idx)})")
    ax.scatter(losses[front_idx], cost[front_idx],
               s=42, color=P_VIOLET, edgecolors="white",
               linewidths=0.7, zorder=4,
               label=f"Pareto front  (n = {len(front_idx)})")
    # Connect the front.
    o = np.argsort(losses[front_idx])
    ax.plot(np.array(losses)[front_idx][o],
            np.array(cost)[front_idx][o],
            color=P_VIOLET, linewidth=1.4, alpha=0.7, zorder=3)

    # Highlight one — the user's chosen design.
    pick_i = front_idx[len(front_idx) // 2]
    ax.scatter([losses[pick_i]], [cost[pick_i]], s=180,
               facecolor=P_AMBER, edgecolors=P_VIOLET_DARK,
               linewidths=2, zorder=5, marker="*",
               label="Selected candidate")

    ax.set_xlabel("Total loss [W]", color=P_TEXT, fontsize=11)
    ax.set_ylabel("Bill-of-materials cost [USD]",
                  color=P_TEXT, fontsize=11)
    ...
    ax.set_title("Pareto optimiser — losses vs cost  ·  1200 candidates",
                 fontsize=12, fontweight="bold", color=P_TEXT,
                 loc="left", pad=10)
    ax.legend(loc="upper left", fontsize=9, frameon=True,
              facecolor=P_SURFACE, edgecolor=P_BORDER,
              labelcolor=P_TEXT)
...
def render_cascade_table(out: Path) -> None:
    ...
    headers = ["Core", "AL [nH]", "N", "L [µH]",
               "Loss T1 [W]", "Loss T2 [W]", "Loss T3 [W]", "ΔT [°C]"]
    ...
    ax.text(0.025, 0.95,
            ...
            fontsize=14, fontweight="bold", color=P_TEXT,
            transform=ax.transAxes)
    ax.text(0.025, 0.91,
            "Loss T3 column shows the FEA-refined value where "
            "available; an em-dash means the candidate hasn't "
            "reached Tier 3 yet.",
            fontsize=8.5, color=P_MUTED, style="italic",
            transform=ax.transAxes)

    # Header row.
    y_top = 0.85
    ax.add_patch(Rectangle(
        (0.025, y_top - cell_h), 0.95, cell_h,
        transform=ax.transAxes,
        facecolor=P_VIOLET_DARK, edgecolor="white", linewidth=0.5,
    ))
    for j, h in enumerate(headers):
        ax.text(cum[j] + col_widths[j] * 0.95 / 2 / sum(col_widths) * 0.95,
                y_top - cell_h / 2, h,
    ...
    for i, row in enumerate(rows):
        ...
        ax.add_patch(Rectangle(
            (0.025, y), 0.95, cell_h,
            transform=ax.transAxes,
            facecolor=bg, edgecolor=P_BORDER, linewidth=0.5,
        ))
        for j, cell in enumerate(row):
            ...
            if i == 0 and j == 0:
                weight = "bold"
                color = P_VIOLET_DARK
            ax.text(cum[j] + 0.005, y + cell_h / 2, txt,
    ...
    ax.text(0.025, 0.04,
            "★ Leader — refined T3 loss 3.02 W, ΔT 18 °C  ·  "
            "click any row to drill into its design.",
            fontsize=9, color=P_VIOLET_DARK,
            transform=ax.transAxes)
...
def render_compare(out: Path) -> None:
    ...
    ax.text(0.02, 0.95, "Compare designs", fontsize=14,
            fontweight="bold", color=P_TEXT)
    ax.text(0.02, 0.90,
            "Variant A trades two more turns for ~6 % less loss "
            "at the same cost as Baseline. Variant B costs 15 % "
            "more without a clear win.",
            fontsize=9, color=P_MUTED, style="italic")

    # Header row with design names.
    col_x = [0.30, 0.51, 0.72]
    col_w = 0.18
    y_header = 0.80
    ax.add_patch(Rectangle((0.02, y_header - 0.06), 0.96, 0.06,
                           facecolor=P_VIOLET_DARK))
    ax.text(0.04, y_header - 0.03, "Metric", fontsize=10,
    ...
    for i, (name, *_) in enumerate(designs):
        ...
        if i == 1:
            ax.add_patch(Rectangle(
                (col_x[i], y_header - 0.06), col_w, 0.06,
                facecolor=bg))
        ax.text(col_x[i] + col_w / 2, y_header - 0.03, name,
    ...
    for ri, (label, fn) in enumerate(metrics):
        ...
        ax.add_patch(Rectangle((0.02, y), 0.96, row_h,
                               facecolor=bg, edgecolor=P_BORDER,
                               linewidth=0.5))
        ax.text(0.04, y + row_h / 2, label, fontsize=10,
                color=P_TEXT, va="center")
        ...
        for i, d in enumerate(designs):
            ...
            if i > 0 and val != baseline_val:
                ...
                color = P_VIOLET_DARK
                weight = "bold"
            ax.text(col_x[i] + col_w / 2, y + row_h / 2, val,
...
def render_export_mockup(out: Path) -> None:
    ...
    ax.add_patch(FancyBboxPatch(
        ...
        linewidth=1, edgecolor=P_BORDER, facecolor="white",
    ))

    # Header.
    ax.add_patch(Rectangle((0, 0.92), 1, 0.08, facecolor=P_VIOLET_DARK))
    ax.text(0.02, 0.96, "INDUCTOR DATASHEET",
            fontsize=12, fontweight="bold", color="white", va="center")
    ax.text(0.02, 0.945, "Boost PFC 1.5 kW · Magnetics 0077439A7 · Rev. A",
            fontsize=8, color="white", va="center", style="italic")
    ax.text(0.98, 0.96, "MagnaDesign", fontsize=10,
    ...
    for i, label in enumerate(("Front", "Top", "Side")):
        ...
        ax.add_patch(Rectangle((x, view_y), view_w, view_h,
                               facecolor=P_BG, edgecolor=P_BORDER))
        # Toroid silhouette inside.
        cx, cy = x + view_w / 2, view_y + view_h / 2
        if i == 1:  # top view — annulus
            ax.add_patch(plt.Circle((cx, cy), 0.075,
                                    facecolor=P_MUTED, alpha=0.6))
            ax.add_patch(plt.Circle((cx, cy), 0.04,
                                    facecolor="white"))
        else:
            ax.add_patch(Rectangle((cx - 0.075, cy - 0.04), 0.15, 0.08,
                                   facecolor=P_MUTED, alpha=0.6))
        ax.text(x + view_w / 2, view_y - 0.02, label,
                fontsize=9, color=P_TEXT, ha="center")
    ...
    table_y = 0.50
    table_h = 0.42
    ax.add_patch(Rectangle((0.04, 0.04), 0.92, table_h,
                           facecolor=P_BG, edgecolor=P_BORDER))
    ax.text(0.06, 0.45, "ELECTRICAL SPECIFICATIONS",
            fontsize=10, fontweight="bold", color=P_VIOLET_DARK)
    for i, (k, v) in enumerate(rows):
        y = 0.41 - i * 0.058
        ax.text(0.06, y, k, fontsize=9, color=P_TEXT)
        ax.text(0.50, y, v, fontsize=9, color=P_TEXT,
                family="monospace")
        if i < len(rows) - 1:
            ax.plot([0.06, 0.94], [y - 0.025, y - 0.025],
                    color=P_BORDER, linewidth=0.5)
...
def render_3d_viewer(out: Path) -> None:
    ...
    z = r * np.sin(v)
    ax.plot_surface(x, y, z, color=P_MUTED, alpha=0.55,
    ...
    ax.set_title(
        ...
        fontsize=11, fontweight="bold", color=P_TEXT, pad=20,
    )
    fig.savefig(str(out), bbox_inches="tight",
                facecolor=fig.get_facecolor(), dpi=110)
    plt.close(fig)
...
def render_logo(out: Path) -> None:
    ...
    ax.set_ylim(0, 1)
    # M-shaped icon
    ax.plot([0.15, 0.15, 0.45, 0.75, 0.75], [0.15, 0.85, 0.5, 0.85, 0.15],
            color=P_VIOLET_DARK, linewidth=3.5, solid_capstyle="round")
    ax.text(1.0, 0.5, "MagnaDesign", fontsize=18,
            fontweight="bold", color=P_TEXT, va="center")
    fig.savefig(str(out), bbox_inches="tight",
...
def main() -> None:
    ...
    from build_screenshots import (
        design_boost_1500w, design_flyback_65w,
    ...
    # ── Spec drawers ──
    render_spec_drawer(
        FIGS / "example1_spec.png", "Boost PFC 1.5 kW",
        [
    ...
    render_spec_drawer(
        FIGS / "example2_spec.png", "3-phase line reactor 22 kW",
        [
            ("V_LL (rms)", "400 V"),
            ("Frequency", "60 Hz"),
            ("P_out", "22 000 W"),
    ...
    render_spec_drawer(
        FIGS / "example3_spec.png", "Flyback DCM 65 W",
        [
            ("V_in (rms)", "85–265 V"),
            ("V_out", "19 V"),
            ("I_out", "3.4 A → 65 W"),
    ...
    v = FEAValidation(
        L_FEA_uH=355.0, L_analytic_uH=358.0, L_pct_error=-0.8,
        B_pk_FEA_T=0.286, B_pk_analytic_T=0.280, B_pct_error=2.1,
        flux_linkage_FEA_Wb=1.50e-3, test_current_A=4.2,
        solve_time_s=8.4, femm_binary="FEMMT (ONELAB) 0.5.5",
        fem_path="/tmp/fea_flyback", log_excerpt="Demo",
