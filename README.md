...
- **[User guide](docs/user-guide/index.rst)** — chapter-by-chapter
  walkthrough of the desktop app with annotated screenshots of every
  workspace tab, dialog, and chart. Start here if you've just installed
  MagnaDesign.
- **[Getting started → first design](docs/getting-started/first-design.rst)** —
  60-second CLI run from a `.pfc` reference project to a datasheet PDF.
- **[Theory of operation](docs/theory/overview.rst)** — Steinmetz/iGSE,
  Dowell, rolloff, thermal, feasibility, compliance. The physics every
  number in the app comes from.
- **[Topology references](docs/topology/)** — boost-CCM,
  line reactor, passive choke, buck-CCM (planned).
- **[Troubleshooting](docs/user-guide/10-troubleshooting.md)** —
  ten common failure modes and their fixes.

The screenshots are auto-generated by
`scripts/generate_docs_screenshots.py` against the example projects in
[`examples/`](examples/). Re-run after a UI change to keep the
