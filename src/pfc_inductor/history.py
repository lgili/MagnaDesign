...
class HistoryStore:
    ...
    def projects(self) -> list[str]:
        ...
        cur = self._conn.execute(
            "SELECT DISTINCT project FROM history ORDER BY project"
        )
    ...
    def delete(self, snapshot_id: int) -> None:
        with self._conn:
            self._conn.execute(
                "DELETE FROM history WHERE id = ?", (snapshot_id,)
            )
    ...
    def clear_project(self, project: str) -> int:
        ...
        with self._conn:
            cur = self._conn.execute(
                "DELETE FROM history WHERE project = ?", (project,)
            )
            return cur.rowcount
...
def diff_snapshots(a: Snapshot, b: Snapshot) -> list[FieldDiff]:
    ...
    out.extend(_diff_dict(a.spec, b.spec, prefix="spec",
                          better_lower=_BETTER_LOWER_KEYS))
    # selection.* — strings only, no numeric ordering.
    out.extend(_diff_dict(a.selection, b.selection, prefix="selection",
                          better_lower=set()))
    ...
    out.extend(_diff_dict(a.summary, b.summary, prefix="summary",
                          better_lower=_BETTER_LOWER_KEYS,
...
_BETTER_LOWER_KEYS: frozenset[str] = frozenset({
    "loss_W", "P_total_W", "T_rise_C", "T_winding_C",
    "mass_g", "cost_USD", "Volume_cm3",
})
_BETTER_HIGHER_KEYS: frozenset[str] = frozenset({
    "eta_pct", "sat_margin_pct",
})
