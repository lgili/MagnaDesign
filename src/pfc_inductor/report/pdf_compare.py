...
def generate_compare_pdf(
    ...
    doc.addPageTemplates(
        [
            PageTemplate(id="default", frames=[frame], onPage=_compare_decoration_factory(fonts)),
        ]
    )
...
def _header(slots: list[CompareSlot], styles, fonts) -> Table:
    ...
    left = [
        ...
        Paragraph("Custom designs — generated by MagnaDesign", styles["subtitle"]),
    ]
    right = [
...
def _spec_table(slots, col_widths, fonts, styles) -> Table:
    ...
    fields = [(label, fn) for (label, fn) in all_fields if any(fn(s) != "—" for s in slots)]
    body_style = styles["body"]
    rows = [[Paragraph(c, body_style) for c in _column_header_row(slots)]]
    for label, fn in fields:
        rows.append(
            [Paragraph(label, body_style)] + [Paragraph(str(fn(s)), body_style) for s in slots]
        )
...
def _metrics_table(slots, col_widths, fonts, styles) -> Table:
    ...
    """
    body_style = styles["body"]
    rows: list[list] = [[Paragraph(c, body_style) for c in _column_header_row(slots)]]
...
def _compare_decoration_factory(fonts):
    def _draw(canvas, doc):
        ...
        canvas.drawString(
            14 * mm,
            8 * mm,
            f"MagnaDesign · {datetime.now().strftime('%Y-%m-%d')} · Comparison",
        )
