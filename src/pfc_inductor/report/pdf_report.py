...
def _build_styles(fonts: dict[str, str]) -> dict[str, ParagraphStyle]:
    ...
    p["badge_ok"] = ParagraphStyle(
        ...
        fontSize=9.5,
        leading=12,
        textColor=_Palette.ok,
        alignment=2,
        backColor=_Palette.ok_lt,
...
def _grid_table_style(fonts: dict[str, str], header_rows: int = 1) -> TableStyle:
    ...
    return TableStyle(
        [
            ...
            ("LINEBELOW", (0, header_rows - 1), (-1, -1), 0.25, _Palette.soft_rule),
            ("VALIGN", (0, 0), (-1, -1), "MIDDLE"),
...
def _spec_data_line_reactor(spec: Spec, result: DesignResult) -> list[tuple[str, str]]:
    ...
    return [
        ...
        ("V line", f"{spec.Vin_nom_Vrms:.0f} {'V_LL' if spec.n_phases == 3 else 'V_LN'}"),
        ...
        ("Pi (active input power)", f"{result.Pi_W:.0f} W" if result.Pi_W else "—"),
    ]
...
# ---------------------------------------------------------------------------
def _bom_data(
    core: Core, wire: Wire, material: Material, result: DesignResult
) -> list[tuple[str, str]]:
    ...
    indent = "&nbsp;&nbsp;"
    return [
        ("Core", f"{core.vendor} — {core.part_number} ({core.shape})"),
        (
            f"{indent}Ae × le × Ve",
            f"{core.Ae_mm2:.0f} mm² × {core.le_mm:.0f} mm × {core.Ve_mm3 / 1000:.1f} cm³",
        ),
        (f"{indent}Wa × MLT", f"{core.Wa_mm2:.0f} mm² × {core.MLT_mm:.0f} mm"),
        ...
        (f"{indent}A_cu / d_cu", f"{wire.A_cu_mm2:.3f} mm² / {wire.d_cu_mm or 0:.2f} mm"),
        (f"{indent}Wire length", f"{wire_len_m:.2f} m"),
        (f"{indent}Wire mass (est.)", f"{wire_mass:.0f}{mass_origin} g"),
    ]


def _tolerance_data(result: DesignResult, material: Material) -> list[tuple[str, str]]:
...
def _build_data(core: Core, wire: Wire, result: DesignResult) -> list[tuple[str, str]]:
...
def _fat_data(
    spec: Spec, result: DesignResult, material: Material
...
def _revision_data(revision: str, designer: str, now: str) -> tuple[list[str], list[list[str]]]:
...
def _metadata_data(
    spec: Spec, core: Core, material: Material, wire: Wire, pn: str
) -> list[tuple[str, str]]:
...
def _grid_flow(
    ...
    body_paras = [[Paragraph(c, body_style) for c in r] for r in rows]
...
def _page1_story(
    ...
    *,
    ...
    spec_grid = Table(
        [
            [
                [Paragraph("Inputs", styles["h3"]), _kv_flow(spec_rows, col_w, fonts, styles)],
                [Paragraph("Computed", styles["h3"]), _kv_flow(res_rows, col_w, fonts, styles)],
            ]
        ],
    ...
    *,
    ...
    if spec.topology == "boost_ccm":
        ...
        if fig_sw is not None:
            curve_blocks.append(
                _section("Switching ripple (Vin_min, peak operating point)", fig_sw)
            )
        ...
        if fig_h is not None:
            curve_blocks.append(
                _section("Harmonic compliance — IEC 61000-3-2 / 61000-3-12 / IEEE 519", fig_h)
            )
    elif spec.topology == "passive_choke":
        fig_cmp = _fig_choke_comparison(spec, result, core)
        if fig_cmp is not None:
            curve_blocks.append(_section("Choke effect — before vs after (estimated)", fig_cmp))
        fig_eta = _fig_efficiency(spec, core, wire, material, result)
        if fig_eta is not None:
            curve_blocks.append(_section("Efficiency vs load", fig_eta))
...
def _page3_story(
    ...
    *,
    ...
    def _kv_section(
        ...
        flowables.append(
            _kv_flow(rows, _USABLE_WIDTH_MM, fonts, styles, label_col_pct=label_col_pct)
        )
    ...
    story.append(
        KeepTogether(
            [
                ...
                # names.
                _grid_flow(fat_header, fat_rows, [50.0, 35.0, 65.0, 32.0], fonts, styles),
            ]
        )
    )
    ...
    story.append(
        KeepTogether(
            [
                ...
                _grid_flow(rev_header, rev_rows, [18.0, 25.0, 35.0, 104.0], fonts, styles),
            ]
        )
    )
...
def generate_pdf_datasheet(
    ...
    doc.addPageTemplates(
        [
            PageTemplate(
                id="default", frames=[frame], onPage=_page_decoration_factory(spec, core, fonts)
            ),
        ]
    )
...
def _header_row(
    ...
    *,
    ...
    left = [
        ...
        Paragraph("Custom design — generated by MagnaDesign", styles["subtitle"]),
    ]
    right = [
...
def _page_decoration_factory(spec: Spec, core: Core, fonts: dict[str, str]):
