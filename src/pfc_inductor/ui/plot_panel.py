...
class PlotPanel(QWidget):
        ...
        *,
        ...
        if is_line_reactor and r.waveform_t_s and r.waveform_iL_A:
            ...
            if r.waveform_t_s and r.waveform_iL_A:
                ...
                ax.text(
                    0.5,
                    0.5,
                    "No waveform (pick boost CCM or line reactor)",
                    ha="center",
                    va="center",
        ...
        if core is not None and material is not None:
            ...
            if tr is not None:
                ...
                ax.scatter(
                    ...
                    edgecolor="white",
                    linewidth=1.5,
                    label=f"Pico ({tr['H_pk_Oe']:.0f} Oe, {tr['B_pk_T'] * 1000:.0f} mT)",
                )
                ax.set_xlabel("H [Oe]")
                ax.set_ylabel("B [mT]")
            ...
            ax.text(
                0.5,
                0.5,
                "Selecione um núcleo e material",
                ha="center",
                va="center",
                transform=ax.transAxes,
                color="#888",
        ...
        if rolloff_curve is not None:
            ...
            ax.text(
                0.5,
                0.5,
                "Material sem rolloff (ferrite/nano sem dados)",
                ha="center",
                va="center",
                transform=ax.transAxes,
            )
    ...
    def _plot_line_reactor(self, r: DesignResult, ax_top, ax_bot) -> None:
        ...
        harmonics_A: dict[int, float] = {
            int(h): (pct_fund[i] / 100.0) * I1_rms for i, h in enumerate(n_axis)
        }
        ...
        bars = ax_bot.bar(plot_orders, plot_amps_mA, width=0.7, color=colors, label="Predito (RMS)")
        ...
        worst_str = f"  ·  pior: h={worst} ({compliance.margin_min_pct:+.1f}%)" if worst else ""
