    ...
    *,
    ...
    rows.append(
        AcceptanceTest(
            ...
            expected=f"{R_ohm * 1000.0:.1f} mΩ" if R_ohm < 1.0 else f"{R_ohm:.3f} Ω",
            tolerance="±15 %",
    ...
    rows.append(
        AcceptanceTest(
            name="Hi-pot",
            condition=(
                f"{hipot_V:.0f} V AC, {insulation.hipot_dwell_s:.0f} s dwell, winding ↔ core"
            ),
    ...
    rows.append(
        AcceptanceTest(
            ...
            expected="No varnish runs, no chipped core, lead positions within ±0.5 mm",
