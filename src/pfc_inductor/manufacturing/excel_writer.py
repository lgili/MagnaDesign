...
def write_mfg_spec_xlsx(spec: MfgSpec, output_path: Path | str) -> Path:
    ...
    wb = Workbook()
    _populate_specs_sheet(wb, spec, Alignment, Border, Font,
                          PatternFill, Side)
    _populate_bom_sheet(wb, spec, Alignment, Border, Font,
                       PatternFill, Side)
    _populate_tests_sheet(wb, spec, Alignment, Border, Font,
                         PatternFill, Side)
...
def _styled_header(Alignment, Border, Font, PatternFill, Side):
    return {
        "fill": PatternFill(start_color=_BAND_BG_HEX,
                            end_color=_BAND_BG_HEX,
                            fill_type="solid"),
        "font": Font(name="Calibri", bold=True, size=10,
                     color=_HEADER_FONT_HEX),
        "border": Border(
...
def _write_data_row(ws, row_idx: int, cells: list,
                    style: dict) -> None:
...
# Sheets
# ---------------------------------------------------------------------------
def _populate_specs_sheet(wb, spec: MfgSpec, Alignment, Border,
                          Font, PatternFill, Side) -> None:
    ...
    rows.extend([
        ...
        ("Mechanical", "OD",              f"{float(spec.core.OD_mm or 0):.2f}",
         "mm", "vendor data"),
        ("Mechanical", "ID",              f"{float(spec.core.ID_mm or 0):.2f}",
         "mm", "vendor data"),
        ("Mechanical", "HT",              f"{float(spec.core.HT_mm or 0):.2f}",
         "mm", "vendor data"),
        ("Mechanical", "Wire",            spec.wire.id, "—", "—"),
        ("Mechanical", "Wire OD",
         f"{_wire_od(spec):.3f}", "mm", "vendor data"),
        ("Mechanical", "N turns",
         str(int(spec.result.N_turns)), "—", "exact"),
        ("Mechanical", "Layers",
         str(spec.winding.n_layers), "—", "exact"),
        ("Mechanical", "Bobbin used",
         f"{spec.winding.bobbin_used_pct:.0f}", "%", "—"),
    ])
    rows.extend([
        ("Electrical", "L_target", f"{spec.result.L_required_uH:.1f}",
         "µH", "±10 %"),
        ("Electrical", "L_actual", f"{spec.result.L_actual_uH:.1f}",
         "µH", "±10 %"),
        ("Electrical", "B_pk", f"{spec.result.B_pk_T * 1000:.0f}",
         "mT", "—"),
        ("Electrical", "P_total", f"{spec.result.losses.P_total_W:.2f}",
         "W", "—"),
        ("Electrical", "T_winding", f"{spec.result.T_winding_C:.1f}",
         "°C", "—"),
        ("Electrical", "T_rise",   f"{spec.result.T_rise_C:.1f}",
         "°C", "—"),
    ])
    rows.extend([
        ("Insulation", "Class",      spec.insulation.name, "—", "IEC 60085"),
        ("Insulation", "T_max",
         f"{spec.insulation.T_max_C:.0f}", "°C", "IEC 60085"),
        ("Insulation", "Inter-layer tape",
         spec.insulation.inter_layer_tape, "—", "—"),
        ("Insulation", "Tape thickness",
         f"{spec.insulation.inter_layer_tape_mm:.2f}", "mm", "—"),
        ("Insulation", "Wire enamel",
         spec.insulation.enamel_grade, "—", "—"),
        ("Insulation", "Hi-pot voltage",
         f"{spec.hipot_V:.0f}", "V AC", "IEC 61558"),
        ("Insulation", "Hi-pot dwell",
...
def _populate_bom_sheet(wb, spec: MfgSpec, Alignment, Border, Font,
                       PatternFill, Side) -> None:
    """Sheet 2 — bill of materials with line totals."""
    ws = wb.create_sheet(title="BOM")
    header = _styled_header(Alignment, Border, Font, PatternFill, Side)
    body = _styled_body(Alignment, Border, Font, PatternFill, Side)
    headers = ["Item", "Vendor PN", "Description", "Qty",
               "Unit", "$/unit", "Line total"]
    _write_header_row(ws, 1, headers, header)
    ...
    wire_length_m = (
        spec.result.N_turns * float(spec.core.MLT_mm or 0) / 1000.0
    )
    ...
    rows.append((
        3,
        f"{spec.insulation.inter_layer_tape} tape "
        f"{spec.insulation.inter_layer_tape_mm:.2f} mm",
        f"{spec.insulation.name} inter-layer dielectric",
        f"{tape_length_m:.2f}", "m",
        "—", "—",
    ))
...
def _populate_tests_sheet(wb, spec: MfgSpec, Alignment, Border, Font,
                         PatternFill, Side) -> None:
    """Sheet 3 — acceptance test plan, one row per test."""
    ws = wb.create_sheet(title="Tests")
    header = _styled_header(Alignment, Border, Font, PatternFill, Side)
    body = _styled_body(Alignment, Border, Font, PatternFill, Side)
    headers = ["#", "Test", "Condition", "Expected",
               "Tolerance", "Instrument"]
    _write_header_row(ws, 1, headers, header)

    for idx, test in enumerate(spec.acceptance_tests, start=1):
        _write_data_row(ws, idx + 1, [
            idx, test.name, test.condition, test.expected,
            test.tolerance, test.instrument,
        ], body)
