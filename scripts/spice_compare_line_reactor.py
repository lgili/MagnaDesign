...
def run_spice(netlist: str, name: str) -> tuple[np.ndarray, np.ndarray]:
    ...
    body = "\n".join(line for line in netlist.splitlines() if line.strip().lower() != ".end")
...
def harmonic_spectrum(
    t: np.ndarray, i: np.ndarray, f_line: float, n_harmonics: int = 15
...
def grab_steady_state(
    t: np.ndarray, y: np.ndarray, f_line: float, n_cycles: int = 5
...
def run_pulsim_3ph(
    ...
    *,
    ...
    for name, node, phase_deg in (("Va", n_a, 0.0), ("Vb", n_b, -120.0), ("Vc", n_c, -240.0)):
    ...
    for name, n_in, n_out in (("La", n_a, n_aL), ("Lb", n_b, n_bL), ("Lc", n_c, n_cL)):
...
def main() -> int:
    ...
    R_load = 620.0
    netlist = netlist_1ph(Vrms_1, f_line, L_H, C_dc_F, R_load, t_stop_s=0.50)
    ...
    spec_1 = Spec(
        ...
        n_phases=1,
        Vin_nom_Vrms=Vrms_1,
        I_rated_Arms=rms_spice,
        pct_impedance=pct_Z,
        f_line_Hz=f_line,
    )
    L_mH = lr.required_inductance_mH(spec_1)
    t_mod, i_mod = lr.line_current_waveform(spec_1, L_mH, n_cycles=5, n_points=len(t_ss))
    ...
    print("   Pulsim ...")
    pulsim_run = run_pulsim_1ph(Vrms_1, f_line, L_H, C_dc_F, R_load, t_stop_s=0.40, dt=20e-6)
    ...
    if pulsim_run is not None:
        ...
        print(
            f"   Pulsim: I_rms = {rms_pulsim:.3f} A   I_pk = {float(np.max(np.abs(i_p_ss))):.2f} A"
        )
        pulsim_pct, thd_pulsim = harmonic_spectrum(t_p_ss, i_p_ss, f_line)

    print(
        f"   THD: spice={thd_spice:.1f}%  model={thd_model:.1f}%"
    ...
    ax_top.plot(
        t_mod * 1000, i_mod, label="modelo analítico", color="#3a78b5", linewidth=1.4, alpha=0.85
    )
    if pulsim_pct is not None:
        ax_top.plot(
            t_p_ss * 1000,
    ...
    ax_bot.legend()
    title_extra = f"  pulsim={thd_pulsim:.1f}%" if pulsim_pct is not None else ""
    ax_bot.set_title(f"Espectro — THD spice={thd_spice:.1f}%  modelo={thd_model:.1f}%{title_extra}")
    ...
    R_load_3 = 144.0
    netlist = netlist_3ph(V_LL, f_line, L_H, C_dc_F_3, R_load_3, t_stop_s=0.50)
    ...
    t_mod, i_mod = lr.line_current_waveform(spec_3, L_mH, n_cycles=5, n_points=len(t_ss))

    spice_pct, thd_spice = harmonic_spectrum(t_ss, i_ss, f_line)
    model_pct, thd_model = harmonic_spectrum(t_mod, i_mod, f_line)

    print("   Pulsim ...")
    pulsim_run = run_pulsim_3ph(V_LL, f_line, L_H, C_dc_F_3, R_load_3, t_stop_s=0.40, dt=20e-6)
    pulsim_pct: np.ndarray | None = None
    thd_pulsim = 0.0
    if pulsim_run is not None:
        t_p, i_p = pulsim_run
    ...
    ax_top.set_title(
        f"3-fase, 380 V_LL, L=10 mH/fase, C={C_dc_F_3 * 1e6:.0f} µF — I_rms ≈ {rms_spice:.2f} A"
    )
    ax_top.grid(True, alpha=0.4)
    ax_top.legend()
    if pulsim_pct is not None:
