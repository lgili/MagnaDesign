...
def _build_record(
    blank: EIBlank, stack_mm: float, material_name: str, material_id: str, mu_eff: float
) -> dict:
    name = f"EI{blank.size}-{round(stack_mm)}"
    geom = _ei_geometry(blank, stack_mm)
    mat_short = material_name.lower().replace("/", "")
    pfc_id = f"dongxing-ei{blank.size}{round(stack_mm)}-{mat_short}"
    return {
        "name": name,
...
def main() -> int:
    ...
    mu_map = {
        m["id"]: m.get("mu_initial", 1.0) for m in materials_data.get("materials", []) if "id" in m
    }
    ...
    for blank in EI_BLANKS:
        for ratio in STACK_RATIOS:
            ...
            for mat_name, mat_id in MATERIALS:
                ...
                new_records.append(_build_record(blank, stack_mm, mat_name, mat_id, mu_eff))

    added_bundled, total_bundled = _merge_into(CORES_FILE, new_records)
    print(
        f"  bundled MAS  {CORES_FILE.relative_to(REPO_ROOT)}: "
    ...
    if user_file is not None and user_file.exists():
        ...
        print(f"  user overlay {user_file}: +{added_user} (total {total_user})")
    elif user_file is not None:
        print(
            f"  user overlay not found at {user_file} — will be seeded from bundled on next launch."
        )
    else:
