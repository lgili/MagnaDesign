...
def reference_inputs():
    ...
    spec_b = spec.model_copy(update={
        "fsw_modulation": FswModulation(
            fsw_min_kHz=8, fsw_max_kHz=20, n_eval_points=4,
        ),
    })
...
def test_modulation_envelope_all_failed_returns_empty_list() -> None:
    ...
    banded = BandedDesignResult(
        spec=spec, band=failing, nominal=None,
        worst_per_metric={}, flagged_points=failing,
    )
    assert modulation_envelope_flowables(banded) == []
...
def test_modulation_envelope_happy_path_carries_pagebreak_and_chart(
    ...
    assert any(
        isinstance(f, Paragraph) and "Modulation envelope" in f.text
        for f in flow
    )
...
def test_modulation_envelope_table_has_metric_rows(
    ...
    assert table._cellvalues[0] == [
        "Metric", "Worst value", "Worst fsw", "Margin",
    ]
...
def test_modulation_envelope_flagged_points_emit_warning() -> None:
    ...
    spec_b = spec.model_copy(update={
        "fsw_modulation": FswModulation(
            fsw_min_kHz=8, fsw_max_kHz=20, n_eval_points=3,
        ),
    })
    mat = next(m for m in mats if m.id == "magnetics-60_highflux")
    core = next(c for c in cores
    ...
    fake_failed = BandPoint(
        fsw_kHz=999.0, result=None, failure_reason="synthetic",
    )
    ...
    assert any(
        ...
        and "1 band point(s) failed" in f.text
        for f in flow
    )
...
def test_worst_case_envelope_empty_summary_returns_empty_list() -> None:
    ...
    summary = WorstCaseSummary(
        ...
        nominal=None, corners=(), worst_per_metric={},
    )
...
def test_worst_case_envelope_happy_path(worst_case_summary) -> None:
    ...
    assert any(
        isinstance(f, Paragraph)
        and "Production worst-case envelope" in f.text
        for f in flow
    )
...
def test_worst_case_envelope_table_has_three_columns(
    ...
    assert len(table._cellvalues) >= 2
    assert table._cellvalues[0] == [
        "Metric", "Worst value", "Driving corner",
    ]
...
def test_worst_case_envelope_yield_section_appears_when_provided(
    ...
    yield_report = YieldReport(
        n_samples=1000, n_pass=970, n_fail=30, n_engine_error=0,
        pass_rate=0.97,
...
def test_worst_case_envelope_yield_section_omits_fail_modes_when_empty(
    ...
    yield_report = YieldReport(
        n_samples=500, n_pass=500, n_fail=0, n_engine_error=0,
        pass_rate=1.0, fail_modes={},
    )
    flow = worst_case_envelope_flowables(worst_case_summary, yield_report)
    assert not any(
        isinstance(f, Paragraph)
...
def test_worst_case_envelope_yield_color_threshold_pass(
    ...
    yield_report = YieldReport(
        n_samples=1000, n_pass=970, n_fail=30, n_engine_error=0,
        pass_rate=0.97, fail_modes={},
    )
    flow = worst_case_envelope_flowables(worst_case_summary, yield_report)
    rate_para = next(
        f for f in flow
        if isinstance(f, Paragraph) and "97.00 %" in f.text
    )
...
def test_worst_case_envelope_yield_color_threshold_warning(
    ...
    yield_report = YieldReport(
        n_samples=1000, n_pass=920, n_fail=80, n_engine_error=0,
        pass_rate=0.92, fail_modes={"T_winding": 80},
    )
    flow = worst_case_envelope_flowables(worst_case_summary, yield_report)
    rate_para = next(
        f for f in flow
        if isinstance(f, Paragraph) and "92.00 %" in f.text
    )
...
def test_worst_case_envelope_yield_color_threshold_fail(
    ...
    yield_report = YieldReport(
        n_samples=1000, n_pass=800, n_fail=200, n_engine_error=0,
        pass_rate=0.80, fail_modes={"T_winding": 200},
    )
    flow = worst_case_envelope_flowables(worst_case_summary, yield_report)
    rate_para = next(
        f for f in flow
        if isinstance(f, Paragraph) and "80.00 %" in f.text
    )
