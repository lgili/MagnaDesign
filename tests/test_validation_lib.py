...
from validation.lib.measure_loader import _to_float, _to_pct  # noqa: E402
...
def test_compare_marks_within_threshold_as_pass(
    ...
    measurements = MeasurementSet(all=[
        # T_winding match
        Measurement(metric="T_winding", condition="d", frequency_Hz=0,
                    value=result.T_winding_C, unit="degC"),
        # P_total match
        Measurement(metric="P_total", condition="d", frequency_Hz=0,
                    value=result.losses.P_total_W, unit="W"),
        # B_pk match
        Measurement(metric="B_pk", condition="d", frequency_Hz=60,
                    value=result.B_pk_T, unit="T"),
        # L match — engine gives µH, bench gives H; the predicted
        # column rescales internally via the `* 1e-6` scale clause
        # in _METRIC_MAP.
        Measurement(metric="L", condition="d", frequency_Hz=1000,
                    value=result.L_actual_uH * 1e-6, unit="H"),
    ])
    thresholds = {
        "inductance_pct": 5.0,
...
def test_compare_flags_out_of_threshold_as_fail(
    ...
    bad = Measurement(metric="P_total", condition="d", frequency_Hz=0,
                      value=result.losses.P_total_W * 0.5, unit="W")
...
def test_compare_handles_missing_measurement_with_skip_note(
    ...
    comparisons, summary = compare(result, MeasurementSet(all=[]),
                                   {"inductance_pct": 5.0})
...
def test_render_summary_reports_pass_or_fail_clearly(
    ...
    spec, core, wire, mat = reference_design
    result = design(spec, core, wire, mat)
    ms = MeasurementSet(all=[
        Measurement(metric="T_winding", condition="d", frequency_Hz=0,
                    value=result.T_winding_C * 5.0, unit="degC"),
    ])
