...
def test_fsw_modulation_uniform_band_validates() -> None:
    ...
    diffs = {round(points[i + 1] - points[i], 6) for i in range(len(points) - 1)}
    assert len(diffs) == 1
...
def test_eval_band_low_fsw_is_thermal_worst_case(
    ...
    assert worst.fsw_kHz <= midband, (
        f"thermal worst case at fsw={worst.fsw_kHz} kHz, expected ≤ {midband} kHz"
    )
...
def test_aggregate_band_picks_worst_per_metric(reference_inputs) -> None:
    ...
    for fsw in (5.0, 12.0, 25.0):
        ...
        points.append(
            BandPoint(
                fsw_kHz=fsw,
                result=run_design(s, core, wire, mat),
            )
        )
    ...
    for metric, bp in banded.worst_per_metric.items():
        assert bp.fsw_kHz in fsw_vals, f"worst[{metric}] picked fsw={bp.fsw_kHz} not in {fsw_vals}"
...
def test_aggregate_band_handles_engine_failures(
    ...
    good = run_design(
        spec.model_copy(update={"f_sw_kHz": 10}),
        core,
        wire,
        mat,
    )
    points = [
        BandPoint(fsw_kHz=4.0, result=None, failure_reason="synthetic"),
...
def test_aggregate_band_edge_weighted_picks_extremes(
    ...
    edge_low = run_design(
        spec.model_copy(update={"f_sw_kHz": 4.0}),
        core,
        wire,
        mat,
    )
    edge_high = run_design(
        spec.model_copy(update={"f_sw_kHz": 25.0}),
        core,
        wire,
        mat,
    )
    centre = run_design(
        spec.model_copy(update={"f_sw_kHz": 14.0}),
        core,
        wire,
        mat,
    )
    ...
    for bp in banded.worst_per_metric.values():
        assert bp.fsw_kHz in (4.0, 25.0), f"edge-weighted aggregator picked centre fsw={bp.fsw_kHz}"
