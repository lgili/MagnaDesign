...
def test_compliance_line_reactor_fails(
    ...
    result = cli_runner.invoke(
        cli,
        ["compliance", str(project_path), "--region", "EU"],
    )
...
def test_compliance_boost_passes_with_caveat(
    ...
    assert "LISN" in iec["summary"] or any("LISN" in n for n in iec["notes"])
...
def test_compliance_us_region_runs_ul1411(
    ...
    result = cli_runner.invoke(
        cli,
        ["compliance", str(project_path), "--region", "US"],
    )
    payload = json.loads(result.stdout)
...
def test_compliance_writes_pdf_to_out_path(
    ...
    result = cli_runner.invoke(
        cli,
        ["compliance", str(project_path), "--region", "EU", "--out", str(pdf_path)],
    )
...
def test_compliance_pretty_mode_renders_human_summary(
    ...
    result = cli_runner.invoke(
        cli,
        ["compliance", str(project_path), "--region", "EU", "--pretty"],
    )
    out = result.output
